...
export default function InformeDietas({ isOpen, onClose, agendas }: Props) {
  ...
  useEffect(() => {
    if (isOpen && aniosDisponibles.length > 0 && !anioSeleccionado) {
      ...
      let mesAUsar = (savedMes !== null && savedMes !== 'todos') ? savedMes : getMonth(new Date()).toString();

      // Ensure anio is selected to valid
      ...
      <DialogContent className="max-w-[95vw] h-[95vh] flex flex-col p-0 bg-white dark:bg-neutral-900 text-black [&>button]:hidden">
        ...
        <DialogHeader className="p-4 border-b flex flex-col lg:flex-row items-center justify-between gap-4 shrink-0 bg-white dark:bg-neutral-900">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 w-full lg:w-auto">
             ...
             <div className="flex gap-2 items-center">
                <Input placeholder="# Informe" className="w-24 h-9 text-xs border-gray-200 dark:border-neutral-700 dark:text-white" value={numeroInforme} onChange={e => setNumeroInforme(e.target.value)} />
             </div>
             ...
             <div className="relative w-full sm:w-28 group">
                ...
                <select value={anioSeleccionado} onChange={(e) => { setAnioSeleccionado(e.target.value); }} disabled={loading || isPrinting} className="w-full pl-9 pr-8 py-2 bg-gray-50 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 dark:text-white rounded-lg text-sm focus:ring-2 outline-none appearance-none">
             ...
             <div className="relative w-full sm:w-36 group">
                ...
                <select value={mesSeleccionado} onChange={(e) => setMesSeleccionado(e.target.value)} disabled={loading || isPrinting} className="w-full pl-9 pr-8 py-2 bg-gray-50 dark:bg-neutral-800 border border-gray-200 dark:border-neutral-700 dark:text-white rounded-lg text-sm focus:ring-2 outline-none appearance-none">
        ...
        <div className="flex-1 overflow-auto bg-gray-100 dark:bg-neutral-950 p-6">
          {loading ? (
            ...
            <div className="bg-white px-10 py-10 w-[816px] mx-auto shadow-sm min-h-[1056px] relative text-black" ref={printRef}>
